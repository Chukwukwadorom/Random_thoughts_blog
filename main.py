...
class BlogPost(db.Model):
    ...
    article_author = relationship("User", back_populates="article")
...
class User(UserMixin, db.Model):
    ...
    article = relationship("BlogPost", back_populates="article_author")
...
# def my_hash(string):
#     import hashlib
#     return hashlib.md5(string.strip().lower())

def admin_only(f):
    @wraps(f)
    def dec_wrapper(*args, **kwargs):
        if current_user.id != 1:
            return abort(403, description="forbidden. go away")
        else:
...
def show_post(post_id):
    form = CommentForm()
    comments = Comment.query.filter_by(article_id=post_id)
    requested_post = BlogPost.query.get(post_id)
    if form.validate_on_submit():
        if current_user.is_active:
            text = remove_html_tags(form.body.data)
            new_comment= Comment(text=text,  comment_author=current_user, article= requested_post)
            db.session.add(new_comment)
            db.session.commit()
            return redirect(url_for("show_post", post_id=post_id))
        else:
            error = "You need to be logged in or registered to comment"
            flash(error)
            return redirect(url_for("login"))
